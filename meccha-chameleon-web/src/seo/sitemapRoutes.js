/**
 * Static routes for sitemap generation — keep in sync with src/pages.
 */
export const sitemapStaticRoutes = [
	{ path: '/', name: 'home', lastmod: '2026-06-25' },
	{ path: '/beginners-hub', name: 'beginners-hub', lastmod: '2026-06-25' },
	{ path: '/wiki', name: 'wiki', lastmod: '2026-06-25' },
	{ path: '/online', name: 'online', lastmod: '2026-06-25' },
	{ path: '/play-game', name: 'play-game', lastmod: '2026-07-13' },
	{ path: '/camouflage-simulator', name: 'camouflage-simulator', lastmod: '2026-06-25' },
	{ path: '/guides', name: 'guides' },
	{ path: '/workshop', name: 'workshop' },
	{ path: '/updates', name: 'updates', lastmod: '2026-06-25' },
	{ path: '/search', name: 'search', lastmod: '2026-06-25' },
	{ path: '/legal/privacy-policy', name: 'legal-privacy' },
	{ path: '/legal/terms-of-service', name: 'legal-terms' },
	{ path: '/legal/copyright', name: 'legal-copyright' },
	{ path: '/legal/about-us', name: 'legal-about' },
	{ path: '/legal/contact-us', name: 'legal-contact' },
];
